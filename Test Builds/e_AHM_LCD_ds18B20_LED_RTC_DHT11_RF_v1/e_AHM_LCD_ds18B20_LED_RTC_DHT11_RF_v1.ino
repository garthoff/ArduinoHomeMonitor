...
// ***************************************************** //   
  ...
  //------------------------------------------------------------
  vw_set_tx_pin(7);          //Transmitter connected to pin 7
...
void loop(void) {
 ...
 // Serial.println(millis() - previous);
   ...
   if (millis() - previousmillis_excel > 1000)
  {
    ...
    int variableA = (int)temp_amb;
     char str[30];  
  
  itoa(variableA,str,10);               //Convert to string
  strcat(str,"A");                      //Add identifier character
  rfWrite(str);                         //Send the string
